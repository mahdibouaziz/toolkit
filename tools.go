...
const randomStringSource = "abcdefgijklmnopstuvwxyzABCDEFGHIJKLMNPQRSTUVWYZ0123456789_"
...
// UploadOneFile upload one file to a specified directory and gives the file a random name
...
// UploadsFiles upload one or more files to a specified directory and gives the files a random name
	...
	for _, fHeaders := range r.MultipartForm.File {
		for _, hdr := range fHeaders {
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				if !allowed {
					return nil, fmt.Errorf("allowed files are: %+v", t.AllowedFileTypes)
				}

				// return to the begining of the file before re reading it from the start
